...
class Games(commands.Cog):
    ...
    async def blackjack(self, ctx: discord.Interaction, currency: app_commands.Choice[str], bet: int):
        ...
        def add_hand_fields(em):
            ...
            em.add_field(name=f"Dealer Hand:", value=f"{config.format_hand(dealer_hand)[0]} | ?", inline=False)
            em.add_field(name="", value=f"**Dealer Hand Value:** {config.single_card_calculation(dealer_hand[0])}{' + ?' * (len(dealer_hand) - 1)}", inline=False)
            return em
